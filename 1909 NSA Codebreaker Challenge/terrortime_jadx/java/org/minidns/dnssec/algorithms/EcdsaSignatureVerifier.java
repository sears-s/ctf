...
abstract class EcdsaSignatureVerifier extends JavaSecSignatureVerifier {
    ...
    public EcdsaSignatureVerifier(BigInteger[] spec2, int length2, String algorithm) throws NoSuchAlgorithmException {
        this(new ECParameterSpec(new EllipticCurve(new ECFieldFp(spec2[0]), spec2[1], spec2[2]), new ECPoint(spec2[3], spec2[4]), spec2[5], 1), length2, algorithm);
    }

    public EcdsaSignatureVerifier(ECParameterSpec spec2, int length2, String algorithm) throws NoSuchAlgorithmException {
        super("EC", algorithm);
        this.length = length2;
        this.spec = spec2;
    }
    ...
    public byte[] getSignature(RRSIG rrsig) throws DataMalformedException {
        ...
        try {
            byte[] r = new byte[this.length];
            dis.readFully(r);
            int rlen = r[0] < 0 ? this.length + 1 : this.length;
            byte[] s = new byte[this.length];
            dis.readFully(s);
            int slen = s[0] < 0 ? this.length + 1 : this.length;
            dos.writeByte(48);
    ...
    public PublicKey getPublicKey(DNSKEY key) throws DataMalformedException, DnssecInvalidKeySpecException {
        ...
        try {
            byte[] xBytes = new byte[this.length];
            dis.readFully(xBytes);
            BigInteger x = new BigInteger(1, xBytes);
            byte[] yBytes = new byte[this.length];
            dis.readFully(yBytes);
            try {
                return getKeyFactory().generatePublic(new ECPublicKeySpec(new ECPoint(x, new BigInteger(1, yBytes)), this.spec));
