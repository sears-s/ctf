...
public class UTF8 {
    ...
    static {
        ...
        fill(bArr, 117, CertificateBody.profileType, 0);
        byte[] bArr2 = transitionTable;
        fill(bArr2, 0, bArr2.length - 1, S_ERR);
        fill(transitionTable, 8, 11, S_END);
        fill(transitionTable, 24, 27, 0);
        fill(transitionTable, 40, 43, 16);
        fill(transitionTable, 58, 59, 0);
        fill(transitionTable, 72, 73, 0);
        fill(transitionTable, 89, 91, 16);
        fill(transitionTable, 104, 104, 16);
        byte[] bArr3 = {0, 0, 0, 0, 31, 15, 15, 15, C_L3C, C_L3C, C_L3C};
        byte[] bArr4 = {S_ERR, S_ERR, S_ERR, S_ERR, 0, S_P3A, 16, S_P3B, S_P4A, S_CS3, S_P4B};
        for (int i = 0; i < 128; i++) {
            byte b = bArr[i];
            byte b2 = bArr3[b] & i;
            firstUnitTable[i] = (short) (bArr4[b] | (b2 << C_L4A));
        }
    }
